...
export function getQuizPacks() {
    ...
    const allHttpCodes = [Informative, Success, Redirection, ClientError, ServerError];
    //console.log("all", allHttpCodes);
    let flatMappedCodes = [];
    allHttpCodes.forEach(codeGroup => {
        codeGroup.codes.forEach(code =>
            flatMappedCodes.push(code)
        )}
    )
...
export function getQuizPack(id) {
    return getQuizPacks().map(quizPack => { params: quizPack.name })[0]
}

export function getQuizNames() {
    const names = getQuizPacks().map(quizPack => { quizPack.name });
    console.log("quizNames", names);
    return names;
}
